...
class LinearFeatureBaseline(nn.Module):
    ...
    def fit(self, states, masks, returns):
        # sequence_length * batch_size x feature_size
        featmat = self._feature(states, masks).view(-1, self.feature_size)
        ...
        for _ in range(5):
            try:
                coeffs, _, _, _ = torch.linalg.lstsq(
                    featmat.t() @ featmat + reg_coeff * eye,
                    featmat.t() @ returns
                )
                break
            except RuntimeError:
