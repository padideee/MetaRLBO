...
from oracles.utils import base_problem
from oracles.utils import domains


def _one_hot(inputs, depth):
  """Wrapper around F.one_hot that passes through one-hot inputs unchanged."""

  if len(inputs.shape) == 3:
    return inputs
  else:
    return F.one_hot(torch.tensor(inputs), num_classes=depth)
...
def _fully_connected_ising_model_energy(variables, potentials):
  ...
  return np.einsum('bim,bjn,ijmn->b', onehot, onehot, potentials)
...
def _locally_connected_ising_model_energy(variables, potentials):
  ...
  return np.einsum('bim,bin,imn->b', oh[:, :-1, :], oh[:, 1:, :], potentials)
  ...
  def __init__(self,
    ...
    if fully_connected:
      self.potentials = tf.constant(
          ...
          dtype=tf.float32)
      self.potentials = torch.randn(size=[length, length, vocab_size, vocab_size]) * weight_variance**0.5 # Mean is 0
    else:
      # self.potentials = tf.constant(
      #     random.normal(
      #         size=[length - 1, vocab_size, vocab_size], scale=weight_variance),
      #     dtype=tf.float32)
      self.potentials = torch.randn(size=[length - 1, vocab_size, vocab_size]) * weight_variance**0.5 # Mean is 0
...
class AlternatingChainIsingModel(base_problem.BaseProblem):
  ...
  def __init__(self, length=4, vocab_size=2, **kwargs):
    ...
    self.potentials = torch.tile(torch.tensor(potentials[np.newaxis, ...]), [length - 1, 1, 1])
