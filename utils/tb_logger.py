...
class TBLogger:
    def __init__(self, config, exp_label):
        self.output_name = exp_label + '_' + str(config["seed"]) + '_' + datetime.datetime.now().strftime('_%d:%m_%H:%M:%S')
        
        log_dir = config["results_log_dir"]
        ...
        try:
            self.full_output_folder = os.path.join(os.path.join(dir_path, 'logs_{}'.format(config["task"])),
                                                   self.output_name)
        except:
            self.full_output_folder = os.path.join(os.path.join(dir_path, 'logs_{}'.format(config["task"])),
                                                   self.output_name)
